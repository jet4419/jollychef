	...
	if Trim(isStoreClosed) = "open" then
		...
		if isProductExist = true then
			...
			if isValidQty = true then
				...
				"VALUES ("&maxID&", 0, "&custID&", "&uniqueNum&", '"&fullName&"', '"&department&"', "&salesOrderID&", "&productID&" , '"&productBrand&"', '"&productName&"', "&price&", "&price&", "&salesQty&", "&salesQty&", "&amount&", "&amount&", "&profit&", "&profit&", '"&status&"', 'false', ctod(["&systemDate&"]))"
